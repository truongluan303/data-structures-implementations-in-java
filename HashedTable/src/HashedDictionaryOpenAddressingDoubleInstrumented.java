...
public class HashedDictionaryOpenAddressingDoubleInstrumented<K,V> implements DictionaryInterface<K,V> {
    ...
    private int getSecondHashIndex(K key) {
        int val = key.toString().hashCode();
        val = Math.abs(val);
        val = val % (hashTable.length - 2) + 1;
        return val;
    } // end getSecondHashIndex
    ...
    {
        boolean found = false;
        int stepSize = getSecondHashIndex(key);
    ...
    private int probe(int index, K key) {
        ...
        int i = 1;
        int secondIndex = getSecondHashIndex(key);
        while (!found && (hashTable[index] != null)) {
            if (hashTable[index].isIn()) {
                if (key.equals(hashTable[index].getKey())) {
                    ...
                    index = (index + i * secondIndex) % hashTable.length; // double probing
                }
            }
                ...
                index = (index + i * secondIndex) % hashTable.length;
            } // end if
            totalProbes++;
        } // end while
